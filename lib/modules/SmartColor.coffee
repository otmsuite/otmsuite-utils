...
# ----------------------------------------------------------------------------
    ...
    # -------------------------------------
        COLOR_REGEXES =
            ...
            RGB:  /([0-9]|[1-9][0-9]|0[0-9][0-9]|0[0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])-([0-9]|[1-9][0-9]|0[0-9][0-9]|0[0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])-([0-9]|[1-9][0-9]|0[0-9][0-9]|0[0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\b/i

            # Matches RGB + A: eg.
            # rgba(0, 99, 199, 0.3) and rgba ( 82   ,    121,    0,     .68  )
            RGBA: /([0-9]|[1-9][0-9]|0[0-9][0-9]|0[0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])-([0-9]|[1-9][0-9]|0[0-9][0-9]|0[0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])-([0-9]|[1-9][0-9]|0[0-9][0-9]|0[0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])-([0-9]|[1-9][0-9]|0[0-9][0-9]|0[0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\b/i
        ...
        MATCH_ORDER = ['HSL', 'HSLA', 'HSV', 'HSVA', 'VEC', 'VECA', 'RGBA', 'RGB', 'HEXA', 'HEX']
    ...
    # -------------------------------------
        n = (number) ->
            ...
            if atom.config.get 'otmsuite-utils.abbreviateValues'
    ...
    # -------------------------------------
        ...
        # -------------------------------------
            ...
            #  RGB
            # ---------------------------
                toRGB: -> return s "#{ @toRGBArray().join '-' }"
                toRGBArray: -> [@RGBAArray[0], @RGBAArray[1], @RGBAArray[2]]

                # RGBA Customizado
                toRGBA: ->
                    _rgbaArray = @toRGBAArray()
                    return s "#{ _rgbaArray[0] }-#{ _rgbaArray[1] }-#{ _rgbaArray[2] }-#{ _rgbaArray[3] }"
                # RGBA no seu formato original
                toRGBA2: ->
                    _rgbaArray = @toRGBAArray()
                    alpha = f _rgbaArray[3]
                    alpha = alpha/255
                    return s "rgba(#{ _rgbaArray[0] }, #{ _rgbaArray[1] }, #{ _rgbaArray[2] }, #{ alpha })"
            ...
            # ---------------------------
                toHEX: ->
                    ...
                    if atom.config.get 'otmsuite-utils.uppercaseColorValues'
        ...
        # -------------------------------------
            ...
            RGBA: (value) -> @color 'RGBA', value, do ->
                ...
                return ([
                    ...
                    parseInt _match[2], 10
                    parseInt _match[3], 10
                    parseInt _match[4], 10
                    ])
            RGBAArray: (value) ->
              @color 'RGBAArray', value, value
